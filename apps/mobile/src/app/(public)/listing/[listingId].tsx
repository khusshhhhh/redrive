...
export default function ListingDetailScreen() {
  ...
  const query = useQuery({ queryKey: ["listing", listingId], queryFn: () => apiRequest<Detail>(`/listings/${listingId}`, { authenticated: status === "authenticated" }), enabled: Boolean(listingId) && status !== "bootstrapping" });
  const favourite = useMutation({ mutationFn: (add: boolean) => apiRequest(`/favourites/${listingId}`, { method: add ? "POST" : "DELETE" }), onSuccess: async (_, add) => { queryClient.setQueryData<Detail>(["listing", listingId], (current) => current ? { ...current, isFavourite: add } : current); await queryClient.invalidateQueries({ queryKey: ["favourites"] }); } });
