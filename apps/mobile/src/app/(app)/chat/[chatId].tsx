...
type MessagePage = { data: Message[]; page: { nextCursor: string | null; hasMore: boolean } };

export default function ChatScreen() {
  const { chatId } = useLocalSearchParams<{ chatId: string }>();
  const client = useQueryClient();
  const [text, setText] = useState("");
  const query = useQuery({ queryKey: ["chat", chatId], enabled: Boolean(chatId), queryFn: () => apiRequest<MessagePage>(`/chats/${chatId}/messages`) });
  ...
  return <SafeAreaView style={styles.safe} edges={["bottom"]}>{query.isPending ? <ActivityIndicator style={styles.loader} color={colors.primary} size="large" /> : query.isError ? <View style={styles.state}><Notice>Messages could not be loaded.</Notice></View> : <FlatList inverted data={query.data?.data || []} keyExtractor={(item) => item.id} contentContainerStyle={styles.list} renderItem={({ item }) => <View style={styles.message}><Text style={styles.messageText}>{item.text || "Image attachment"}</Text><Text style={styles.time}>{new Intl.DateTimeFormat("en-AU", { hour: "numeric", minute: "2-digit" }).format(new Date(item.createdAt))}</Text></View>} ListEmptyComponent={<Text style={styles.empty}>Start the conversation.</Text>} />}<View style={styles.composer}><TextInput value={text} onChangeText={setText} placeholder="Write a message" placeholderTextColor={colors.muted} multiline style={styles.input} /><Pressable disabled={!text.trim() || send.isPending} onPress={() => send.mutate(text.trim())} style={styles.send}><Text style={styles.sendText}>{send.isPending ? "…" : "Send"}</Text></Pressable></View>{send.isError ? <Text style={styles.error}>Message was not sent. Try again.</Text> : null}</SafeAreaView>;
}

const styles = StyleSheet.create({ safe: { flex: 1, backgroundColor: colors.background }, loader: { flex: 1 }, state: { padding: spacing.lg }, list: { padding: spacing.md, gap: spacing.sm }, message: { alignSelf: "stretch", backgroundColor: colors.surface, borderWidth: 1, borderColor: colors.border, borderRadius: radii.md, padding: spacing.md, gap: spacing.xs }, messageText: { color: colors.ink, lineHeight: 20 }, time: { color: colors.muted, fontSize: 11 }, empty: { color: colors.muted, textAlign: "center", padding: spacing.xl }, composer: { flexDirection: "row", alignItems: "flex-end", gap: spacing.sm, padding: spacing.md, borderTopWidth: 1, borderTopColor: colors.border, backgroundColor: colors.surface }, input: { flex: 1, maxHeight: 120, minHeight: 44, borderWidth: 1, borderColor: colors.border, borderRadius: radii.md, color: colors.ink, padding: spacing.sm }, send: { minHeight: 44, justifyContent: "center", borderRadius: radii.md, backgroundColor: colors.primary, paddingHorizontal: spacing.md }, sendText: { color: "#FFFFFF", fontWeight: "800" }, error: { color: colors.danger, paddingHorizontal: spacing.md, paddingBottom: spacing.sm, backgroundColor: colors.surface } });
