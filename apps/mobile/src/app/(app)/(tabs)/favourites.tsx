...
type ListingPage = { data: PublicListing[]; page: { nextCursor: string | null; hasMore: boolean } };

export default function FavouritesScreen() {
  const query = useQuery({ queryKey: ["favourites"], queryFn: () => apiRequest<ListingPage>("/favourites") });
...
const styles = StyleSheet.create({ safe: { flex: 1, backgroundColor: colors.background }, header: { padding: spacing.lg, paddingBottom: spacing.sm, gap: spacing.xs }, title: { color: colors.ink, fontSize: 30, fontWeight: "800" }, detail: { color: colors.muted }, loader: { flex: 1 }, state: { padding: spacing.lg, gap: spacing.md }, list: { padding: spacing.lg, gap: spacing.md }, card: { flexDirection: "row", backgroundColor: colors.surface, borderWidth: 1, borderColor: colors.border, borderRadius: radii.lg, overflow: "hidden" }, image: { width: 112, minHeight: 112, backgroundColor: colors.border }, cardBody: { flex: 1, padding: spacing.md, gap: spacing.xs }, cardTitle: { color: colors.ink, fontSize: 17, fontWeight: "800" }, meta: { color: colors.muted }, price: { color: colors.primary, fontWeight: "800" }, empty: { color: colors.muted, textAlign: "center", padding: spacing.xl } });
