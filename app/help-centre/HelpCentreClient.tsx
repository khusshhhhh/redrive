    ...
    <>
      <section className="bg-ink text-white">
        <div data-info-reveal className="mx-auto max-w-[1120px] px-5 py-14 sm:px-8 sm:py-20 lg:px-10">
          ...
          <label className="mt-8 flex max-w-2xl items-center gap-3 rounded-md bg-white px-4 py-3.5 text-ink shadow-card">
...
function SupportCard({ icon, title, copy, href }: { icon: React.ReactNode; title: string; copy: string; href: string }) {
  return <Link href={href} className="rounded-md bg-surface-soft p-5 transition hover:bg-surface-strong"><span className="text-primary">{icon}</span><h3 className="mt-4 font-semibold text-ink">{title}</h3><p className="mt-2 text-sm leading-6 text-muted">{copy}</p></Link>;
}
