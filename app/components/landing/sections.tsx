    ...
    <section className="border-y border-hairline-soft bg-surface-soft/60">
      <div className="mx-auto grid max-w-6xl grid-cols-2 gap-px overflow-hidden px-5 py-2 sm:px-8 lg:grid-cols-4">
        {[
          ...
          <Reveal key={i} delay={i * 70} className="px-2 py-8 text-center">
...
function CategoryChip({ label, Icon }: { label: string; Icon: () => React.ReactNode }) {
  return (
    <span className="inline-flex shrink-0 items-center gap-2.5 rounded-full border border-hairline bg-white px-5 py-3 text-sm font-semibold text-ink shadow-[0_6px_18px_-12px_rgba(59,59,59,0.4)]">
      <Icon />
      {label}
    </span>
  );
  ...
  return (
    <section className="overflow-hidden py-14 lg:py-20">
      <div className="mx-auto mb-8 max-w-6xl px-5 sm:px-8">
        <p className="text-[11px] font-bold uppercase tracking-[0.18em] text-primary">Whatever the plan</p>
        <h2 className="mt-3 text-display-2xl font-extrabold tracking-tight text-ink">One marketplace, every kind of vehicle.</h2>
      </div>
...
export function HowItWorks() {
  ...
  return (
    <section className="mx-auto max-w-6xl px-5 py-16 sm:px-8 lg:py-28">
      <Reveal>
        <p className="text-[11px] font-bold uppercase tracking-[0.18em] text-primary">How Redrive works</p>
      ...
      <div className="mt-14 grid gap-4 md:grid-cols-3">
        {steps.map((step, i) => (
          <Reveal
            key={step.n}
            delay={i * 110}
            className="group relative overflow-hidden rounded-2xl border border-hairline-soft bg-white p-7 transition hover:-translate-y-1 hover:border-border-strong hover:shadow-[0_24px_60px_-30px_rgba(59,59,59,0.5)]"
          >
            <span className="text-[13px] font-black tracking-widest text-hairline transition group-hover:text-border-strong">
              {step.n}
            </span>
      ...
      >
        ...
        <div className="relative max-w-xl">
          ...
          <h2 className="mt-4 text-display-3xl font-extrabold tracking-tight">
            Your vehicle could be working while you&rsquo;re not.
          </h2>
