...
const RegisterModal = () => {
  ...
  const footerContent = <div className="mx-5 mb-5 mt-2 flex flex-col gap-3 sm:mx-7"><div className="relative"><div className="absolute inset-0 flex items-center"><div className="w-full border-t border-hairline-soft" /></div><div className="relative flex justify-center"><span className="bg-white px-3 text-[11px] uppercase tracking-widest text-muted-soft">or</span></div></div><Button outline label="Continue with Google" icon={FcGoogle} onClick={async () => { await signIn("google", { callbackUrl: "/profile" }); }} /><div className="mt-1 text-center"><span className="text-sm text-muted">Already have an account? </span><button type="button" onClick={toggle} className="cursor-pointer text-sm font-semibold text-ink hover:underline">Log in</button></div></div>;
