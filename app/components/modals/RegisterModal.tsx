...
const RAIL_POINTS: RailPoint[] = [
  { icon: <UserRound size={14} />, text: "One account to book vehicles and to list your own." },
  { icon: <KeyRound size={14} />, text: "Email verification now; licence check later, from your profile." },
  { icon: <ShieldCheck size={14} />, text: "Australian marketplace — payments held safely until handover." },
];
...
const RegisterModal = () => {
  ...
  const onAddressSelect = (result: ParsedAddress) => {
    if (result.state) {
      const nextState =
        AU_STATES.find((item) => item.value === result.state) || { value: result.state, label: result.state };
      setSelectedState(nextState);
      setValue("state", nextState.value, { shouldValidate: true });
    }
    if (result.suburb) {
      const nextSuburb: SuburbOption = {
        value: result.suburb,
  ...
  const selectSuburb = (selected: SuburbOption) => {
    ...
    if (selected.state) {
      ...
      if (stateOption) {
        ...
        setValue("state", stateOption.value, { shouldValidate: true });
      }
    }
  };
  ...
  const nextStep = async () => {
    const fields =
      stage === "account"
        ...
        : ["number", "dateOfBirth", "streetAddress", "suburb", "state"];
  ...
  const onSubmit: SubmitHandler<FieldValues> = async (data) => {
    ...
    try {
      ...
      if (
        axios.isAxiosError<{ code?: string; emailVerified?: boolean; error?: string }>(error) &&
        error.response?.status === 409 &&
        error.response?.data?.code === "EMAIL_ALREADY_REGISTERED"
      ) {
  ...
  const completeProfile = async () => {
    ...
    await axios.put("/api/profile", {
      ...
      hobbies: data.hobbies
        ? data.hobbies.split(",").map((item: string) => item.trim()).filter(Boolean)
        : [],
      dreamDestinations: data.dreamDestinations
        ? data.dreamDestinations.split(",").map((item: string) => item.trim()).filter(Boolean)
        : [],
    });
    if (profileResult[0].status === "rejected") {
      toast.error("Your account is ready, but the profile image could not be uploaded. You can retry from your profile.");
    }
  };
  ...
  const resendCode = async () => {
    ...
    try {
      ...
      toast.error(
        axios.isAxiosError<{ error?: string }>(error)
          ? error.response?.data?.error || "Unable to resend code"
  ...
  const accountContent = (
    <div className="space-y-4">
      ...
      <Input
        ...
        errors={errors}
        required
        validate={(value: string) => /^\S+@\S+\.\S+$/.test(value.trim()) || "Enter a valid email address"}
        onChange={() => {
          if (existingEmailNotice) {
            setExistingEmailNotice(null);
            clearErrors("email");
          }
        }}
      />
      {existingEmailNotice && (
        <div role="status" className="rounded-md border border-hairline bg-surface-soft p-4">
          <div className="flex gap-3">
            ...
            <div>
              <p className="text-sm font-semibold text-ink">
                {existingEmailNotice.verified
                  ? "You already have a Redrive account"
                  : "Your signup is already in progress"}
              </p>
              <p className="mt-1 text-xs leading-5 text-muted">
                {existingEmailNotice.verified
                  ? `${existingEmailNotice.email} is already registered. Sign in with your existing password to continue.`
                  : `We found an unverified signup for ${existingEmailNotice.email}. Send a new verification code to continue safely.`}
              </p>
              <button
                type="button"
                onClick={existingEmailNotice.verified ? toggle : continueExistingVerification}
                className="mt-3 inline-flex items-center gap-2 text-xs font-semibold text-primary hover:underline"
              >
                {existingEmailNotice.verified ? <LogIn size={14} /> : <Mail size={14} />}
                {existingEmailNotice.verified ? "Sign in instead" : "Send verification code"}
              </button>
            </div>
          </div>
        </div>
  ...
  const profileContent = (
    <div className="space-y-4">
      <SignupImagePicker
        ...
        description="Help hosts recognise you at handover."
        value={profileImage}
      ...
      <Input
        ...
        errors={errors}
        required
        validate={(value: string) => isValidAustralianMobile(value) || "Enter a valid Australian mobile number"}
      />
      <p className="flex gap-2 text-xs leading-5 text-muted">
        <Smartphone size={14} className="mt-0.5 shrink-0" />
        We save your number securely. SMS ownership verification needs a delivery provider and is not enabled yet.
      </p>
      <div>
        <label htmlFor="signup-date-of-birth" className="mb-2 block text-xs font-semibold text-ink">
          Date of birth
        </label>
        <input
          type="hidden"
          {...register("dateOfBirth", {
            ...
            validate: (value: string) =>
              (isValidDateOfBirth(value) && isAtLeast18(value)) || "You must be at least 18",
          })}
        />
        <DatePicker
        ...
        {errors.dateOfBirth && (
          <p className="mt-1 text-xs text-error">
            {String(errors.dateOfBirth.message || "Enter your date of birth")}
          </p>
        )}
      </div>
      <AddressAutocomplete
      ...
      <input type="hidden" {...register("suburb", { required: true })} />
      <input type="hidden" {...register("state", { required: true })} />
      {(errors.suburb || errors.state) && <p className="text-xs text-error">Choose your suburb and state.</p>}
  ...
  const aboutContent = (
    <div className="space-y-4">
      <Input id="hobbies" label="Hobbies, separated by commas" disabled={isLoading} register={register} errors={errors} />
      <Input
        id="dreamDestinations"
        label="Dream destinations, separated by commas"
        disabled={isLoading}
        register={register}
        errors={errors}
      />
      <div className="flex gap-3 rounded-md border border-amber-200 bg-amber-50 p-4 text-xs leading-5 text-amber-900">
        <ShieldCheck size={17} className="mt-0.5 shrink-0" />
        <span>
          After signup, open Profile to photograph both sides of your Australian driver licence. Booking
          remains locked until its details and expiry match your profile.
        </span>
      </div>
    </div>
  );
  ...
  const verifyContent = (
    <div className="flex flex-col items-center text-center">
      <div className="mb-4 flex h-12 w-12 items-center justify-center rounded-full bg-surface-soft text-primary">
        <Mail size={23} />
      </div>
      ...
      <input
        ...
        placeholder="000000"
        className="mt-6 w-full rounded-md border border-hairline bg-white px-4 py-4 text-center text-2xl font-semibold tracking-[0.45em] text-ink outline-none transition placeholder:text-hairline focus:border-ink focus:ring-1 focus:ring-ink"
      />
      {previewCode && (
      ...
      <p className="mt-5 text-xs text-muted">
        Code expires in 10 minutes. Didn&rsquo;t get it?{" "}
        <button
          type="button"
          disabled={isLoading}
          onClick={resendCode}
      ...
      <button
        ...
        className="mt-3 text-xs text-muted hover:text-ink hover:underline"
      >
  ...
  const successContent = (
    <div className="flex flex-col items-center py-4 text-center">
      ...
      <p className="mt-2 max-w-sm text-sm leading-6 text-muted">
        Your email and profile details are saved. Add and check your Australian driver licence from Profile
        before requesting a booking.
      </p>
    </div>
  );

  const belowAction = stage === "account" ? (
  ...
  const subtitle =
    stage === "account"
      ? "It takes about two minutes. Email verification is next."
      : stage === "profile"
        ? "Where you'll pick up vehicles, and how hosts reach you."
        : stage === "about"
          ? "Optional details that help hosts get to know you."
          : undefined;
  ...
  const stepNumber = stage === "account" ? 1 : stage === "profile" ? 2 : stage === "about" ? 3 : undefined;
  ...
  return (
    <AuthShell
      ...
      disabled={isLoading}
      loading={isLoading}
      eyebrow={stepNumber ? `Step ${stepNumber} of 3` : undefined}
      progress={stepNumber}
      title={title}
      subtitle={subtitle}
      actionLabel={actionLabel}
      onBack={stage === "profile" || stage === "about" ? previousStep : undefined}
      railHeadline="Join Redrive — rent a useful vehicle, or earn from the one in your driveway."
