...
export async function sendVerificationEmail(email: string, code: string) {
  ...
  await transport.transporter.sendMail({
    ...
    html: `
      <div style="font-family:Arial,sans-serif;max-width:520px;margin:auto;padding:32px;color:#16323A">
        <div style="font-size:22px;font-weight:700;margin-bottom:28px;color:#087E8B">Redrive</div>
        <h1 style="font-size:24px;margin:0 0 12px">Verify your email</h1>
        <p style="color:#506B72;line-height:1.6">Enter this code to finish creating your account. It expires in ${CODE_TTL_MINUTES} minutes.</p>
        <div style="font-size:34px;font-weight:700;letter-spacing:10px;background:#F1F8F8;border:1px solid #D7E7E8;border-radius:12px;padding:20px;text-align:center;margin:24px 0">${code}</div>
  ...
  await transport.transporter.sendMail({
    ...
    html: `
      <div style="font-family:Arial,sans-serif;max-width:520px;margin:auto;padding:32px;color:#16323A">
        ...
        <p style="color:#506B72;line-height:1.6">Enter this code to complete your login. It expires in ${CODE_TTL_MINUTES} minutes.</p>
        <div style="font-size:34px;font-weight:700;letter-spacing:10px;background:#F1F8F8;border:1px solid #D7E7E8;border-radius:12px;padding:20px;text-align:center;margin:24px 0">${code}</div>
