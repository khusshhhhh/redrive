...
export function TrendChart({ monthly }: { monthly: Monthly }) {
  ...
  return <section className="rounded-2xl border border-black/5 bg-white p-5 shadow-sm sm:p-6">
    ...
    <div className="mt-5 overflow-x-auto"><svg viewBox={`0 0 ${width} ${height}`} role="img" aria-label={`${labels[metric]} over the last ${months} months`} className="min-w-[620px]">
      <defs><linearGradient id="adminArea" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stopColor="#1F1F1F" stopOpacity=".24"/><stop offset="1" stopColor="#1F1F1F" stopOpacity="0"/></linearGradient></defs>
      ...
      <path d={area} fill="url(#adminArea)"/><path d={line} fill="none" stroke="#1F1F1F" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"/>
      {points.map((point) => <g key={point.key} className="group"><circle cx={point.x} cy={point.y} r="11" fill="transparent"/><circle cx={point.x} cy={point.y} r="4" fill="white" stroke="#1F1F1F" strokeWidth="3"/><g className="opacity-0 transition group-hover:opacity-100"><rect x={point.x-34} y={Math.max(0, point.y-34)} width="68" height="24" rx="7" fill="#1F1F1F"/><text x={point.x} y={Math.max(16, point.y-18)} textAnchor="middle" fill="white" fontSize="11" fontWeight="600">{metric === "revenue" ? `$${point[metric].toLocaleString()}` : point[metric]}</text></g><text x={point.x} y={height-12} textAnchor="middle" fill="#454545" fontSize="11">{point.label}</text></g>)}
    </svg></div>
  </section>;
}

export function StatusDonut({ statuses, total }: { statuses: AdminDashboardData["statuses"]; total: number }) {
  const colours = ["#1F1F1F", "#9C9C9C", "#454545", "#808080", "#6E6E6E", "#9C9C9C"];
