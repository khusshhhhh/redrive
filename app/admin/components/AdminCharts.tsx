...
export function TrendChart({ monthly }: { monthly: Monthly }) {
  ...
  return <section className="rounded-2xl border border-black/5 bg-white p-5 shadow-sm sm:p-6">
    ...
    <div className="mt-5 overflow-x-auto"><svg viewBox={`0 0 ${width} ${height}`} role="img" aria-label={`${labels[metric]} over the last ${months} months`} className="min-w-[620px]">
      <defs><linearGradient id="adminArea" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stopColor="#710014" stopOpacity=".24"/><stop offset="1" stopColor="#710014" stopOpacity="0"/></linearGradient></defs>
      {[0, .25, .5, .75, 1].map((ratio) => <line key={ratio} x1={padX} x2={width-padX} y1={padTop + ratio*(height-padTop-padBottom)} y2={padTop + ratio*(height-padTop-padBottom)} stroke="#edeae4" strokeDasharray="4 6" />)}
      <path d={area} fill="url(#adminArea)"/><path d={line} fill="none" stroke="#710014" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"/>
      {points.map((point) => <g key={point.key} className="group"><circle cx={point.x} cy={point.y} r="11" fill="transparent"/><circle cx={point.x} cy={point.y} r="4" fill="white" stroke="#710014" strokeWidth="3"/><g className="opacity-0 transition group-hover:opacity-100"><rect x={point.x-34} y={Math.max(0, point.y-34)} width="68" height="24" rx="7" fill="#161616"/><text x={point.x} y={Math.max(16, point.y-18)} textAnchor="middle" fill="white" fontSize="11" fontWeight="600">{metric === "revenue" ? `$${point[metric].toLocaleString()}` : point[metric]}</text></g><text x={point.x} y={height-12} textAnchor="middle" fill="#8A6849" fontSize="11">{point.label}</text></g>)}
    </svg></div>
  </section>;
}

export function StatusDonut({ statuses, total }: { statuses: AdminDashboardData["statuses"]; total: number }) {
  const colours = ["#710014", "#B38F6F", "#8A6849", "#A3121F", "#8F877F", "#C6C0B6"];
