...
export default function AdminLoginForm() {
  ...
  return (
    <form onSubmit={submit} className="w-full rounded-2xl border border-hairline-soft bg-white p-6 shadow-card sm:p-8">
      ...
      <button disabled={isLoading || (stage === "otp" && otp.length !== 6)} className="mt-6 flex w-full items-center justify-center gap-2 rounded-lg bg-primary px-4 py-3 text-sm font-semibold text-white transition hover:bg-primary-active disabled:cursor-not-allowed disabled:opacity-60">{isLoading ? <Loader2 size={17} className="animate-spin" /> : <LockKeyhole size={17} />} {isLoading ? "Signing in…" : stage === "otp" ? "Verify and open dashboard" : "Open admin dashboard"}</button>
