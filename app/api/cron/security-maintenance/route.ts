...
async function GETHandler(request: Request) {
  ...
  const now = new Date();
  const [rateLimits, resetTokens, sessions, mobileSessions, mobileChallenges, idempotencyRecords, licences, apiMetrics, apiErrors] = await prisma.$transaction([
