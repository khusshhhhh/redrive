...
import { claimStripeEvent, releaseStripeEvent } from "@/app/libs/stripeWebhookEvents";

export const runtime = "nodejs";
// A few branches call back into the Stripe API (`paymentIntents.retrieve`) plus
// ledger writes; give the function real headroom instead of the platform default.
...
async function processEvent(event: Stripe.Event): Promise<void> {
  ...
  if (event.type === "checkout.session.expired") {
    ...
    await prisma.payment.updateMany({
      where: { stripeCheckoutSessionId: session.id, status: "CHECKOUT_PENDING" },
  ...
  if (event.type === "payment_intent.payment_failed") {
    ...
    if (intent.metadata?.kind === "trip_extension") return;
    ...
    await prisma.payment.updateMany({
      where: reservationId ? { reservationId } : { stripePaymentIntentId: intent.id },
      data: {
        status: "PAYMENT_FAILED",
        failureMessage:
          intent.last_payment_error?.message?.slice(0, 500) || "Payment failed",
      },
    });
    ...
    if (failedReservation && failedReservation.status === "APPROVED") {
      await notificationService
        .notifyPaymentFailed(failedReservation.userId, failedReservation.listing.title, reservationId!)
        .catch((notifyError) => console.error("Payment-failed notification failed", notifyError));
    }
  }
}
...
async function POSTHandler(request: Request) {
  ...
  try {
    ...
    logger.warn("stripe_webhook_bad_signature", { err: error });
    return NextResponse.json({ error: "Invalid signature" }, { status: 400 });
  }

  // Claim the event before any processing so concurrent deliveries and retries
  ...
  try {
    ...
    captureException(error, {
      ...
      fields: { eventId: event.id, type: event.type },
      tags: { stripe_event_type: event.type },
    });
    // Drop the claim so Stripe's retry can re-attempt a transient failure.
