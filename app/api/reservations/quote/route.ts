...
async function POSTHandler(request: Request) {
  ...
  const listing = await prisma.listing.findUnique({ where: { id: body.listingId }, select: { price: true, cleaningFeeOption: true, cleaningFeeAmount: true } });
  ...
  return NextResponse.json({ ...quote, expiresAt: new Date(Date.now() + 15 * 60_000).toISOString() }, { headers: { "Cache-Control": "no-store" } });
}
