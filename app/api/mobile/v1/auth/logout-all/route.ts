...
async function POSTHandler(request: Request) {
  ...
  const [result] = await Promise.all([
    revokeAllMobileSessions(auth.identity.userId, "LOGOUT_ALL"),
    prisma.mobilePushToken.updateMany({ where: { userId: auth.identity.userId, disabledAt: null }, data: { disabledAt: new Date() } }),
  ]);
