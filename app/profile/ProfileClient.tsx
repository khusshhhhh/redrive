...
import AddressAutocomplete, { type ParsedAddress } from "@/app/components/inputs/AddressAutocomplete";
import PasswordField, { isStrongPassword } from "@/app/components/inputs/PasswordField";
import ImageUpload from "@/app/components/inputs/ImageUpload";
import SuburbSelector, { type SuburbOption } from "@/app/components/inputs/SuburbSelector";
...
const SectionCard = ({ id, icon, title, description, children }: {
  ...
  <section id={id} className="scroll-mt-32 rounded-md border border-hairline-soft bg-white p-5 sm:p-8">
    <div className="mb-7 flex items-start gap-4">
      <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-surface-soft text-ink">{icon}</div>
      <div>
...
export default function ProfileClient({ initialUser }: { initialUser: SafeUser & { hasPassword: boolean } }) {
  ...
  const [image, setImage] = useState(initialUser.image || "/images/placeholder.png");
  const [licenseImage, setLicenseImage] = useState(initialUser.licenseImage || "");
  const [licenseType, setLicenseType] = useState(initialUser.licenseType || "Driver License");
  ...
  const [selectedState, setSelectedState] = useState<{ value: string; label: string } | null>(() => {
    ...
    return AU_STATES.find((item) => item.value === initialUser.state) || { value: initialUser.state, label: initialUser.state };
  });
  const [selectedSuburb, setSelectedSuburb] = useState<SuburbOption | null>(() => initialUser.suburb ? ({
    value: initialUser.suburb,
    label: initialUser.postcode ? `${initialUser.suburb}, ${initialUser.postcode}` : initialUser.suburb,
    postcode: initialUser.postcode ? Number(initialUser.postcode) : undefined,
    state: initialUser.state || undefined,
  }) : null);
  const [destinations, setDestinations] = useState<string[]>(initialUser.dreamDestinations || []);
  ...
  const selectSuburb = (selected: SuburbOption) => {
    ...
    setValue("postcode", selected.postcode?.toString() || "", { shouldDirty: true });
    if (selected.state) {
  ...
  const addDestination = () => {
    ...
    if (!value || destinations.some((item) => item.toLowerCase() === value.toLowerCase())) return;
  ...
  const saveProfile: SubmitHandler<ProfileFormData> = async (data) => {
    ...
    try {
      await axios.put("/api/profile", {
        ...
        licenseImage,
        licenseType,
        hobbies: data.hobbies ? data.hobbies.split(",").map((item) => item.trim()).filter(Boolean) : [],
  ...
  const changePassword: SubmitHandler<PasswordFormData> = async (data) => {
    ...
    try {
      ...
      toast.error(error.response?.data?.error || "Password could not be updated");
    } finally {
    ...
    <main className="bg-surface-soft/40 px-4 py-8 sm:px-6 sm:py-12">
      <div className="mx-auto max-w-[1120px]">
        <header className="mb-9 flex flex-col justify-between gap-5 sm:flex-row sm:items-end">
          <div>
            <p className="mb-2 text-xs font-semibold uppercase tracking-[0.16em] text-primary">Account</p>
        ...
        <div className="grid items-start gap-7 lg:grid-cols-[280px_1fr]">
          <aside className="space-y-5 lg:sticky lg:top-32">
            <div className="rounded-md border border-hairline-soft bg-white p-6 text-center">
              <div className="relative mx-auto h-28 w-28">
                <div className="h-full w-full overflow-hidden rounded-full border border-hairline bg-surface-soft">
                  <img src={image} alt={name || "Profile"} width={112} height={112} decoding="async" className="h-full w-full object-cover" />
                </div>
            ...
            <nav className="hidden rounded-md border border-hairline-soft bg-white p-2 lg:block">
              {[["personal", "Personal details"], ["address", "Address"], ["about", "About you"], ["verification", "Verification"], ["security", "Login & security"]].map(([href, label]) => (
                <a key={href} href={`#${href}`} className="block rounded-sm px-4 py-3 text-sm font-medium text-muted transition hover:bg-surface-soft hover:text-ink">{label}</a>
              ))}
            </nav>
          </aside>
          ...
          <div className="space-y-6">
            <form onSubmit={handleSubmit(saveProfile)} className="space-y-6">
              ...
              <SectionCard id="address" icon={<MapPin size={19} />} title="Home address" description="Start with your street address—we’ll fill the suburb and state when possible.">
                <div className="space-y-4">
                  <AddressAutocomplete id="streetAddress" label="Number & street address" register={register} setValue={setValue} errors={errors} onSelect={onAddressSelect} />
                  <div className="grid gap-4 sm:grid-cols-2">
                    <div>
                      <label className="mb-2 block text-xs font-semibold text-ink">Suburb</label>
                      <SuburbSelector state={selectedState?.value} value={selectedSuburb || undefined} allowAllStates onChange={selectSuburb} />
                    </div>
                    <div>
                      <label className="mb-2 block text-xs font-semibold text-ink">State</label>
                      <StateSelector value={selectedState} onChange={selectState} />
                    </div>
                  </div>
                  <input type="hidden" {...register("suburb")} />
                  <input type="hidden" {...register("state")} />
                  <input type="hidden" {...register("postcode")} />
                  {selectedSuburb?.postcode && <p className="text-xs text-muted">Postcode {selectedSuburb.postcode} will be saved with this address.</p>}
                </div>
              </SectionCard>

              <SectionCard id="about" icon={<Heart size={19} />} title="A little about you" description="Add details that make conversations and handovers more personal.">
                <div className="space-y-5">
                  <Input id="hobbies" label="Hobbies, separated by commas" register={register} errors={errors} />
                  <div>
                    <label htmlFor="destination" className="mb-2 block text-xs font-semibold text-ink">Dream destinations</label>
                    <div className="flex gap-2">
                      <input id="destination" value={destinationInput} onChange={(event) => setDestinationInput(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === ",") { event.preventDefault(); addDestination(); } }} placeholder="Type a place and press Enter" className="h-12 min-w-0 flex-1 rounded-sm border border-hairline bg-white px-4 text-sm text-ink outline-none transition focus:border-ink focus:ring-1 focus:ring-ink" />
                      <button type="button" onClick={addDestination} className="rounded-sm border border-ink px-4 text-sm font-semibold text-ink transition hover:bg-surface-soft">Add</button>
                    </div>
                    <div className="mt-3 flex flex-wrap gap-2">
                      {destinations.map((destination) => <span key={destination} className="inline-flex items-center gap-2 rounded-full bg-surface-soft px-3 py-2 text-xs text-ink">{destination}<button type="button" onClick={() => removeDestination(destination)} aria-label={`Remove ${destination}`} className="text-muted hover:text-ink"><X size={13} /></button></span>)}
                      {!destinations.length && <span className="text-xs text-muted">No destinations added yet.</span>}
                    </div>
                  </div>
                </div>
              </SectionCard>
              ...
              <div className="divide-y divide-hairline-soft rounded-md border border-hairline-soft">
                ...
                <div className="p-5">
                  <div className="flex items-center justify-between gap-5">
                    <div><h3 className="text-sm font-semibold text-ink">Password</h3><p className="mt-1 text-xs leading-5 text-muted">{hasPassword ? "Use a unique password you don’t use elsewhere." : "This account currently signs in with Google."}</p></div>
                    {hasPassword && <button type="button" onClick={() => setShowPasswordForm((current) => !current)} className="shrink-0 text-sm font-semibold text-ink underline underline-offset-4">{showPasswordForm ? "Cancel" : "Update"}</button>}
                  </div>
                  {hasPassword && showPasswordForm && (
                    <form onSubmit={passwordForm.handleSubmit(changePassword)} className="mt-6 space-y-4 rounded-md bg-surface-soft p-4 sm:p-5">
                      <PasswordField id="currentPassword" label="Current password" autoComplete="current-password" register={passwordForm.register} errors={passwordForm.formState.errors} />
                      <PasswordField id="newPassword" label="New password" autoComplete="new-password" register={passwordForm.register} errors={passwordForm.formState.errors} valueForStrength={newPassword || ""} showRequirements validate={(value) => isStrongPassword(value) || "Complete all password requirements"} />
                      <PasswordField id="confirmPassword" label="Confirm new password" autoComplete="new-password" register={passwordForm.register} errors={passwordForm.formState.errors} validate={(value) => value === newPassword || "Passwords do not match"} />
                      <div className="pt-1 sm:ml-auto sm:w-44"><Button type="submit" label="Change password" loading={isChangingPassword} loadingLabel="Updating" /></div>
                    </form>
                  )}
                </div>
              </div>
            ...
            <div className="flex items-center gap-3 rounded-md bg-ink p-5 text-white sm:p-6">
              <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-white/10"><Sparkles size={19} /></div>
              <div><p className="text-sm font-semibold">A stronger profile builds trust</p><p className="mt-1 text-xs leading-5 text-white/70">Complete details, a clear photo and login verification help both sides feel ready before a trip.</p></div>
            </div>
          </div>
        </div>
      </div>
